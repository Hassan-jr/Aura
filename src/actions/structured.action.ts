...
const ContentStructure = z.object({
  PostTitle: z
    .string()
    .describe(
      "A catchy and engaging title for a social media post (max 100 characters)."
    ),
  PostDescription: z
    .string()
    .describe(
  ...
  Prompt2: z
    .string()
    .describe(
      "Another detailed image prompt (around 75-100 words), starting EXACTLY with '[TOKEN], a photo of [TOKEN]'. Describe a DIFFERENT scene, potentially focusing on a different key feature or benefit, or a contrasting environment."
    ),
  Prompt3: z
    .string()
    .describe(
      "A third detailed image prompt (around 75-100 words), starting EXACTLY with '[TOKEN], a photo of [TOKEN]'. Explore another distinct visual concept."
    ),
  Prompt4: z
    .string()
    .describe(
      "A fourth detailed image prompt (around 75-100 words), starting EXACTLY with '[TOKEN], a photo of [TOKEN]'. Offer a final unique visual angle."
    ),
});
...
export async function generateStructuredContent(
  ...
  console.log(
    `[generateStructuredContent] Starting content generation for token: ${tokenName} and product: ${productTitle}`
  );

  // 1. Check for OpenAI API Key
  if (!process.env.OPENAI_API_KEY) {
    console.error(
      "[generateStructuredContent] OpenAI API key is not configured in environment variables."
    );
  ...
  const feedbackString =
    Array.isArray(feedbackTexts) && feedbackTexts.length > 0
  ...
  try {
    ...
    if (!generatedContent || typeof generatedContent !== "object") {
      console.error(
        "[generateStructuredContent] Parsed content is not a valid object or is null/empty.",
        generatedContent
      );
    ...
    console.log(
      "[generateStructuredContent] Successfully received and parsed content from OpenAI:",
      generatedContent
    );
    ...
    console.error("--------------------------------------------------");
    console.error(
      "[generateStructuredContent] FAILED: Error during OpenAI API call or parsing."
    );
    ...
    console.error("--------------------------------------------------");
    throw new Error(
      `[generateStructuredContent] Failed to generate/parse content from OpenAI. Reason: ${error.message}`
    );
  }
}
